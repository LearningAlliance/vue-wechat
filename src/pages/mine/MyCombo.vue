...
<template>
  <div class="page">
    <div class="section-tab clearfix">
      <div class="tab-left">
        <div :class="['tab-item', {'active': activedTab == 'a'}]" @click="clickTab('a')">进行中</div>
      </div>
      <div class="tab-right">
        <div :class="['tab-item', {'active': activedTab == 'b'}]" @click="clickTab('b')">已完成</div>
      </div>
    </div>
    <div class="blank"></div>
            ...
            <div class="box" @click="toDetail(item.orderNo, item.createDate)">
              ...
              <div class="box-content clearfix">
                ...
                <div class="box-center">
                  <div class="box-shopname">{{item.productName}}</div>
...
export default {
  ...
  methods: {
    clickTab(tab) {
      this.activedTab = tab;
      if (tab == 'a') {
        this.searchCondition.orderState = '-1';
      } else if (tab == 'b') {
        this.searchCondition.orderState = '-2';
      }
      this.loadTop();
    },
    ...
    loadPageList() {
      // 查询数据
      api.trade.getOrderInfoCombo(this.searchCondition).then((res) => {
