            ...
            <div class="box">
              <div class="box-content clearfix">
                ...
                <div class="box-center">
                  <div class="box-shopname invalid">{{item.shopName}}</div>
                  <div class="box-price clearfix">有效期至：{{item.expDate.slice(0,10)}}</div>
                </div>
                <span class="box-price-other invalid">{{item.couponName}}</span>
                <p class="box-status invalid"><span class="currency invalid">￥</span>{{Number(item.couponPrice).toFixed(2)}}</p>
              </div>
...
export default {
  ...
  methods: {
    ...
    loadPageList() {
      // 查询数据
      api.user.getInvalidVoucher(this.searchCondition).then((res) => {
