...
<template>
  <div class="page">
    <div class="section">
      <div class="box">
        <div :class="['cell', {'selected': item.selected}]" v-for="(item, index) in list" @click="selectCell(index)" :key="index">
          ...
          <p class="text-2" v-show="item.credits">兑换{{item.credits}}积分</p>
    ...
    <div class="readMe">
      <p>说明：</p>
      <p>1.保障金兑换成积分后，将无法再兑换回保障金，请谨慎操作；</p>
      <p>2.当前兑换比例为 1保障金=100积分；</p>
    </div>
    <div class="footer clearfix">
      <div class="footer-desc">{{!!value ? value : 0}}保障金兑换{{value * 100}}积分</div>
